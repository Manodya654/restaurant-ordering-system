...
const AdminOrders = () => {
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      pending: { color: 'bg-yellow-50 text-yellow-700 border-yellow-100', text: 'Pending', icon: Clock },
      confirmed: { color: 'bg-blue-50 text-blue-700 border-blue-100', text: 'Confirmed', icon: CheckCircle },
      preparing: { color: 'bg-purple-50 text-purple-700 border-purple-100', text: 'Preparing', icon: ChefHat },
      ready: { color: 'bg-green-50 text-green-700 border-green-100', text: 'Ready for Pickup', icon: Package },
      completed: { color: 'bg-green-50 text-green-700 border-green-100', text: 'Completed', icon: CheckCircle },
      cancelled: { color: 'bg-red-50 text-red-700 border-red-100', text: 'Cancelled', icon: XCircle },
    };
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm border ${statusInfo.color}`}>
        <Icon size={16} />
        {statusInfo.text}
      </span>
    );
    ...
    <>
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
              <select
                ...
                className="px-4 py-2 border border-orange-100 bg-white rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              >
                      ...
                      <tr key={order._id} className="hover:bg-orange-50/30 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <button
                            ...
                            className="text-orange-600 hover:text-orange-900 font-medium flex items-center gap-1 p-2 hover:bg-orange-50 rounded-lg transition-colors"
                          >
                            <Eye size={16} />
                            View
                          </button>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-orange-100 shadow-2xl">
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-4 bg-[#FFF7ED] p-4 rounded-xl border border-orange-50">
                  <div>
              ...
              <div className="mb-6">
                ...
                <div className="space-y-3">
                  {selectedOrder.items.map((item, index) => (
                    <div key={index} className="flex items-center gap-4 bg-gray-50/50 p-3 rounded-xl border border-gray-100">
                      {item.image && (
              ...
              <div className="mb-6">
                <div className="flex justify-between text-lg font-bold border-t border-gray-100 pt-4">
                  <span>Total</span>
                  <span className="text-orange-600">LKR {selectedOrder.totalAmount.toLocaleString()}</span>
                </div>
                <div className="mt-2">
              ...
              {selectedOrder.specialInstructions && (
                <div>
                  ...
                  <p className="text-gray-700 bg-gray-50 p-4 rounded-xl border border-gray-100">{selectedOrder.specialInstructions}</p>
                </div>
              )}
            </div>
          </div>
